...
function CartItem({id, title, price, image, description, count, totalprice}) {
    ...
    return (
        <tr className="grid grid-cols-4 text-left mt-[2rem] lg:px-[1.5rem] gap-[0.5rem] lg:gap-[1rem]">
            <td className="flex flex-col lg:flex-row gap-[0.5rem] lg:gap-[3rem] col-span-2 items-center">
                <img src={image} alt="" className="h-[4rem] w-[3rem] lg:w-[5rem] lg:h-[6rem] "/>
                <div>
                    <h4 className="font-semibold text-[#0f1111] text-[0.8rem] lg:text-[1rem]">{title}</h4>
                    <p className="text-gray-400 text-[0.8rem] lg:text-[1rem]">${price}</p>
                </div>
            </td>
            <td className="flex flex-col-reverse lg:flex-row items-center gap-[0.5rem] justify-center lg:justify-normal lg:gap-[1rem]">
                <button className="rotate-[-90deg] lg:rotate-0 text-gray-400 lg:text-[1.5rem] font-bold bg-gray-200 p-[3px]"
